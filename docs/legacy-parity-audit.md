...
| Remote desktop monitor/capture/input | `GetMonitors`, `GetDesktop`, `GetDesktopResponse`, `DoMouseEvent`, `DoKeyboardEvent`, `FrmRemoteDesktop` | partial | Monitor count, single-frame capture, and input done; continuous delta streaming/render loop and session UI/active-state parity pending. |
...
1. Continuous remote desktop streaming and operator render loop.
