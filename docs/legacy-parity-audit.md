...
| Remote desktop monitor/capture/input | `GetMonitors`, `GetDesktop`, `GetDesktopResponse`, `DoMouseEvent`, `DoKeyboardEvent`, `FrmRemoteDesktop` | partial | Monitor count, single-frame capture, input, and reusable request-response frame streaming done; GUI/live viewer, resize-aware rendering, visible active state, and richer session controls pending. |
