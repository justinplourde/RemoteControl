...
| Remote desktop screen capture | `GetDesktop`, `GetDesktopResponse`, `GetMonitors`, `GetMonitorsResponse` | monitor count and single-frame capture done; continuous streaming deferred | Windows-specific provider today | sensitive capture | redesign | `GetMonitors` returns monitor count; `GetDesktop` returns a legacy first-frame JPEG payload and CLI saves it locally; continuous streaming still needs client-visible status, consent policy, session controls, and audit |
