...
| Remote shell | `DoShellExecute`, `DoShellExecuteResponse` | one-command dispatch path done; persistent interactive shell pending | platform-specific | sensitive execution | redesign | Requires execution permission plus client consent; current CLI returns combined stdout/stderr for one command, while legacy-style persistent shell sessions still need session/receive model work |
