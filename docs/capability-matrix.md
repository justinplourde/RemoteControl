...
| Remote desktop screen capture | `GetDesktop`, `GetDesktopResponse`, `GetMonitors`, `GetMonitorsResponse` | monitor count, single-frame capture, and CLI request-loop streaming done | Windows-specific provider today | sensitive capture | partial | `GetMonitors` returns monitor count; `GetDesktop` returns a legacy first-frame JPEG payload; CLI `get-desktop-stream` repeats the legacy request-response frame loop and saves numbered JPEGs. GUI/live viewer, visible active state, session controls, and audit remain |
