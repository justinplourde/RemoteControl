...
| File download from client / file upload to client | `FileTransferRequest`, `FileTransferChunk`, `FileTransferComplete`, `FileTransferCancel` | download done and manually verified; upload protocol only | portable with platform-specific filesystem constraints | sensitive data movement | keep | Download requires file-read permission, streams bounded chunks, refuses overwrite, and was SHA-256 verified on loopback; upload still needs operator permission, audit, size limits, path policy, cancellation and progress model |
