...
| TCP connection close | `DoCloseConnection` | dispatch path done; elevated close verification pending | Windows-specific today | state-changing network | keep | Requires operator permission; manual non-elevated loopback dispatch returned refreshed connections but did not close the target row, so admin/elevation handling and success reporting need follow-up |
