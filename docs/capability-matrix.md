...
| Remote desktop screen capture | `GetDesktop`, `GetDesktopResponse`, `GetMonitors`, `GetMonitorsResponse` | monitor count, single-frame capture, reusable request-loop streaming, first WinForms viewer surface, monitor refresh UX, viewer input dispatch, viewer active/session state, local GUI verification, and keyboard cleanup done | Windows-specific provider today | sensitive capture | partial | `GetMonitors` returns monitor count; `GetDesktop` returns a legacy first-frame JPEG payload; server core owns reusable request-response stream sequencing; CLI `get-desktop-stream` saves numbered JPEGs; WinForms viewer can listen, select a client, refresh displays, select a monitor, start/stop stream, render frames, show FPS, expose state/client/status/access/session/last-frame details, and send mouse/keyboard input with zoom-aware coordinate scaling. The viewer releases tracked remote key-down events and local modifiers on stream teardown. June 7, 2026 local GUI verification streamed `1280x720` at 801 frames / 21.60 FPS with plain-key input and clean stop. Audit persistence remains |
...
| Reverse proxy | `ReverseProxyConnect`, `ReverseProxyData`, `ReverseProxyDisconnect`, `ReverseProxyConnectResponse` | first client runtime path done; operator SOCKS listener pending | portable transport, platform networking varies | sensitive network routing | partial | Modern client handlers can connect, relay data, and disconnect through a loopback-only target policy; dispatch remains behind `NetworkControl` permission. Broader target allowlists, operator SOCKS5/HTTPS listener, load balancing, session controls, audit persistence, and GUI parity remain |
