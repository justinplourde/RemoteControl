...
| File delete and rename | `DoPathDelete`, `DoPathRename`, `SetStatusFileManager` | rename done and manually verified; delete protocol only | portable with platform-specific permissions | state-changing filesystem | keep | Rename requires file-write permission, refuses target overwrite, and was hash-verified on loopback; delete still needs stronger confirmation policy and protected path rules |
