    ...
    internal sealed class HostOptions
    {
        ...
        public static HostOptions Parse(string[] args)
        {
            ...
            for (int index = 0; index < args.Length; index++)
            {
                ...
                {
                    throw new InvalidOperationException("Usage: LocationRemote.Client.Host [--host 127.0.0.1] [--port 4782] [--client-id <64 chars>] [--tag modern] [--encryption-key dev-key] [--smoke-test]");
                }
                else
                {
